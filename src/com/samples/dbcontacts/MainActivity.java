...
public class MainActivity extends ListActivity {
	...
	private static final String[] mContent = new String[] {
			FlightDbHelper._ID, FlightDbHelper.DESTINATION,
			FlightDbHelper.FLIGHT_NUM };

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);

		/*SQLiteDatabase db = ContactProvider.getDb();
		 ContactDbHelper d = new ContactDbHelper(getBaseContext());
		 d.onUpgrade(db, 0, 0);*/

		 mCursor = managedQuery(FlightProvider.CONTENT_URI, mContent, null,
				null, null);
		...
		mAdapter = new SimpleCursorAdapter(
				...
				R.layout.main,
				mCursor,
				new String[] { FlightDbHelper.DESTINATION,
						FlightDbHelper.FLIGHT_NUM},
				new int[] { R.id.destination, R.id.flight_num });
	...
	private void CallDeleteContactDialog(final long id) {
		...
		builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				getContentResolver().delete(FlightProvider.CONTENT_URI,
	...
	private void CallEditContactDialog(long id) {
		...
		final EditText destET = (EditText) root.findViewById(R.id.destET);
		final EditText flightNumET = (EditText) root.findViewById(R.id.flightNumET);
		final EditText planeTypeET = (EditText) root.findViewById(R.id.planeTypeET);

		AlertDialog.Builder b = new AlertDialog.Builder(this);
		b.setView(root);
		b.setTitle(R.string.title_edit);
		String[] col = { "DESTINATION", "FLIGHT_NUM", "PLANE_TYPE" };
		Cursor c = getContentResolver().query(FlightProvider.CONTENT_URI, col,
		...
		b.setPositiveButton(R.string.btn_ok,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						values.put(FlightDbHelper.DESTINATION, destET
								.getText().toString());
						values.put(FlightDbHelper.FLIGHT_NUM, flightNumET
								.getText().toString());
						values.put(FlightDbHelper.FLIGHT_NUM, planeTypeET
								.getText().toString());

						getContentResolver().update(
								FlightProvider.CONTENT_URI, values,
								"_ID=" + selectedIndex, null);
						mCursor.requery();
					}
				});
		...
		b.setPositiveButton(R.string.btn_ok,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						...
						values.put(FlightDbHelper.FLIGHT_NUM, flightNumET
								.getText().toString());
						values.put(FlightDbHelper.PLANE_TYPE, planeTypeET
								.getText().toString());
